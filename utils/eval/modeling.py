...
class LlmAdapterModel(SeqToSeqModel):
    use_template: bool = False
    """
    Not officially supported by AutoModelForCausalLM, so we need the specific class
    Optionally, we can use the prompt template from: https://github.com/tatsu-lab/stanford_alpaca/blob/main/train.py
    ...
    def count_text_length(self, text: str) -> int:
        # self.load()
        return self.tokenizer(text, return_tensors="pt")["input_ids"].size(-1)
    ...
    def run(self, prompt: str, **kwargs) -> str:
        ...
        #print("🌹 The length of the input tokens: {}".format(input_ids.size(-1)))
        #temperature=0.1,
        #    top_p=0.75,
        #    top_k=40,
        #    num_beams=4,
        ...
        answer = self.tokenizer.decode(outputs[0][inputs["input_ids"].size(-1):]).strip()
        return answer
    ...
    def get_choice(self, text: str, **kwargs) -> Tuple[float, float]:
        ...
        A_index = self.tokenizer("A", add_special_tokens=False).input_ids[0]
        B_index = self.tokenizer("B", add_special_tokens=False).input_ids[0]
        C_index = self.tokenizer("C", add_special_tokens=False).input_ids[0]
        D_index = self.tokenizer("D", add_special_tokens=False).input_ids[0]
        A = float(predictions[A_index].cpu())
        B = float(predictions[B_index].cpu())
        C = float(predictions[C_index].cpu())
        D = float(predictions[D_index].cpu())
        return A, B, C, D
